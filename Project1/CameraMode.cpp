...
void TpsCameraMode::Update(GameCamera * Camera, PanzerPilot* Panzer)
{
	...
	Math::Vector3& CameraPosition = Camera->GetTransform().GetPosition();
	auto offset = Math::Vector3(0.0f, 5.0f, 0.0f);
	CameraPosition = Panzer->GetTransform().GetPosition() + (Panzer->GetTransform().GetVector(Transform::Vector::Backward) * Panzer->GetTpsOffset()) + offset;
	Math::Vector3 target = Panzer->GetTransform().GetPosition() + (Panzer->GetTransform().GetVector(Transform::Vector::Forward) * Panzer->GetTargetOffset());
...
void FpsCameraMode::Update(GameCamera * Camera, PanzerPilot* Panzer)
{
	...
	Math::Vector3& CameraPosition = Camera->GetTransform().GetPosition();
	CameraPosition = Panzer->GetTransform().GetPosition() + (Panzer->GetTransform().GetVector(Transform::Vector::Forward) * Panzer->GetFpsOffset());
	Math::Vector3 target = Panzer->GetTransform().GetPosition() + (Panzer->GetTransform().GetVector(Transform::Vector::Forward) * Panzer->GetTargetOffset());
